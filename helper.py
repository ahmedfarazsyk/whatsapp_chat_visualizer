...
def fetch_stats(selected_user, df):
    ...
    return num_messages, len(words), media_files, len(links)
...
def fetch_most_busy_users(df):
    ...
    df = round((df["user"].value_counts()/df.shape[0])*100, 2).reset_index().rename(columns = {"count":"percentile"})
...
def most_common_words(selected_user, df):
    ...
    common_words = pd.DataFrame(Counter(words).most_common(20)).sort_values(by = [1])
...
def monthly_timeline(selected_user, df):
    ...
    time = timeline["month"] + " - " + timeline["year"].apply(lambda x: str(x))
